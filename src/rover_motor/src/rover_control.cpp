...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    switch (key) {
      case 'w': case 'W': write_all("F\n"); break;
      case 's': case 'S': write_all("B\n"); break;
      case 'a': case 'A': write_all("L\n"); break;
      case 'd': case 'D': write_all("R\n"); break;
      case ' ': write_all("S\n"); break;
      case '[': left_pwm = std::max(0, left_pwm - 5); set_pwm(left_pwm, right_pwm); break;
      case ']': left_pwm = std::min(255, left_pwm + 5); set_pwm(left_pwm, right_pwm); break;
      case '-': right_pwm = std::max(0, right_pwm - 5); set_pwm(left_pwm, right_pwm); break;
      case '=': right_pwm = std::min(255, right_pwm + 5); set_pwm(left_pwm, right_pwm); break;
      case 'p': case 'P':
